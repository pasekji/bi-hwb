...
inline uint32_t word(uint8_t a0, uint8_t a1, uint8_t a2, uint8_t a3) // returns whole word into uint32
{
...
// source: https://github.com/secana/AES-TBOX
void buildTBoxes()
{
	for(int i = 0; i < 256; i++)
	{
		TBox0[i] = word(xtime(SBOX[i]), SBOX[i], SBOX[i], (SBOX[i] ^ xtime(SBOX[i])));
		TBox1[i] = word((SBOX[i] ^ xtime(SBOX[i])), xtime(SBOX[i]), SBOX[i], SBOX[i]);
		TBox2[i] = word(SBOX[i], (SBOX[i] ^ xtime(SBOX[i])), xtime(SBOX[i]), SBOX[i]);
		TBox3[i] = word(SBOX[i], SBOX[i], (SBOX[i] ^ xtime(SBOX[i])), xtime(SBOX[i]));
	}
}
...
{	
	...
	uint32_t t0 = TBox0[tempArray[0]] ^ TBox1[tempArray[5]] ^ TBox2[tempArray[10]] ^ TBox3[tempArray[15]];
	uint32_t t1 = TBox0[tempArray[4]] ^ TBox1[tempArray[9]] ^ TBox2[tempArray[14]] ^ TBox3[tempArray[3]];
	uint32_t t2 = TBox0[tempArray[8]] ^ TBox1[tempArray[13]] ^ TBox2[tempArray[2]] ^ TBox3[tempArray[7]];
	uint32_t t3 = TBox0[tempArray[12]] ^ TBox1[tempArray[1]] ^ TBox2[tempArray[6]] ^ TBox3[tempArray[11]];
	
	tempArray[0] = t0 & 0xff;
	tempArray[1] = (t0 >> 8) & 0xff;
	tempArray[2] = (t0 >> 16) & 0xff;
	tempArray[3] = (t0 >> 24) & 0xff;

	tempArray[4] = t1 & 0xff;
	tempArray[5] = (t1 >> 8) & 0xff;
	tempArray[6] = (t1 >> 16) & 0xff;
	tempArray[7] = (t1 >> 24) & 0xff;

	tempArray[8] = t2 & 0xff;
	tempArray[9] = (t2 >> 8) & 0xff;
	tempArray[10] = (t2 >> 16) & 0xff;
	tempArray[11] = (t2 >> 24) & 0xff;

	tempArray[12] = t3 & 0xff;
	tempArray[13] = (t3 >> 8) & 0xff;
	tempArray[14] = (t3 >> 16) & 0xff;
	tempArray[15] = (t3 >> 24) & 0xff;

	s[0] = word(tempArray[0], tempArray[1], tempArray[2], tempArray[3]);
	s[1] = word(tempArray[4], tempArray[5], tempArray[6], tempArray[7]);
	s[2] = word(tempArray[8], tempArray[9], tempArray[10], tempArray[11]);
	s[3] = word(tempArray[12], tempArray[13], tempArray[14], tempArray[15]);
